...
No agent may `git push` to any repo without passing validation first. Applies to herv3, salad, nanoclaw, and agent state files.

```
commit locally → validate → security check → git push
```

Full validation commands per project: load `memory/pipeline.md` before any push.
Security check: no hardcoded secrets, no PHI exposure, no auth regressions, no debug flags left on.
